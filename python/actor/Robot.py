...
def _smooth_moves(x, y, segments=3):
    ...
    for i in range(segments - 1):
        x_range = [0, (x - np.sum(x_moves))]
        x_moves.append(np.random.randint(min(x_range), max(x_range)))
    x_moves.append(x - np.sum(x_moves))

    y_moves = []
    for i in range(segments - 1):
        y_range = [0, (y - np.sum(y_moves))]
        y_moves.append(np.random.randint(min(y_range), max(y_range)))
...
def shoot():
    ...
    if StateManager.shooting:
        ...
        # print(time.time() - StateManager.start_time)
        time.sleep(np.random.uniform(.001, .003))
        ...
        if StateManager.spray_mode or not StateManager.shot:
            if StateManager.beast_mode() or (StateManager.scope in [ReferenceManager.Scope.x1t]):
                ...
                if current_target.confidence > .5:
                    x_delta_pred, y_delta_pred, time_ratio = None, None, None
                    ...
                    move(x, y)
                    shoot()
                    # print(current_target.x, current_target.y, x_delta_pred, y_delta_pred, time_ratio)
